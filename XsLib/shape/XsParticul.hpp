...
private:
	...
	XsClock __tempTime, __create;
	std::deque<XsClock> __time;
public:
	float lifeTime;
	float reCreate;
	...
	void draw(GLenum mode) {
		if (__create.getMilliSeconds() > reCreate) {
			...
			for (int __i = 0; __i < density; __i++) {
				...
				__pos.push_back(pos + (XsRand3f(random.x, random.y) / random.z));
			}
		}
			glLoadIdentity();
		for (int _i = 0; _i < __pos.size(); _i++) {
			...
			glBegin(mode);
			XsPrintVertex(vertices, vex3cb(0, 1, 1), __pos[_i]);
			glEnd();
	...
	void draw(XsVertexDepo vertices, GLenum mode) {
		if (__create.getMilliSeconds() > reCreate) {
			...
			for (int __i = 0; __i < density; __i++) {
				__tempTime.start();
				__time.push_back(__tempTime);
				__pos.push_back(pos + (XsRand3f(posRandom.x, posRandom.y) / posRandom.z));
				__vel.push_back(velocity);
				__color.push_back(color + (XsRand3f(colorRandom.x, colorRandom.y) / colorRandom.z));
			};
		}
		glLoadIdentity();
		for (int _i = 0; _i < __pos.size(); _i++) {
			...
			glBegin(mode);
			XsPrintVertex(vertices, vex3cb(0, 1, 1), vex3f(1),__pos[_i]);
			glEnd();
			if (__time[_i].getMilliSeconds() > lifeTime) {
				__time.pop_front();
				__pos.pop_front();
