...
void XsSolidTriangle() {
	...
	for (float i = -0.9; i < 0.9; i += 0.333)
		glVertex3f(cos(i * XS_PI), sin(i * XS_PI), 0);
	glEnd();
};
...
void XsSolidCube(XsEnum mode) {
	...
	case XS_ALL:
		...
		glEnableClientState(GL_TEXTURE_COORD_ARRAY);
		glEnableClientState(GL_NORMAL_ARRAY);
		glVertexPointer(3, GL_FLOAT, 8 * sizeof(GLfloat), XsCubeNormalArray);
		glTexCoordPointer(2, GL_FLOAT, 8 * sizeof(GLfloat), XsCubeTexturedArray + 3);
		glNormalPointer(GL_FLOAT, 8 * sizeof(GLfloat), XsCubeNormalArray + 5);
		glDrawArrays(GL_TRIANGLES, 0, 36);
		glDisableClientState(GL_VERTEX_ARRAY);
		glDisableClientState(GL_NORMAL_ARRAY);
		glDisableClientState(GL_TEXTURE_COORD_ARRAY);
...
void XsSolid(XsEnum _shape, XsEnum mode) {
	switch (_shape) {
	case XS_CUBE:
		XsSolidCube(mode);
		break;
