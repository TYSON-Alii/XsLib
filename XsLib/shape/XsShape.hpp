...
void XsVerticesLoad(XsVertices& _a, XsVertexDepo _v, XsEnum _mode) {
	...
	case XS_VERTEX:
		_a.resize(_v.size() * 3);
		for (volatile size_t _i = 0; _i < _v.size(); _i++) {
...
public:
	XsShape() {};
	XsShape(const char* loadfromfile) {
	...
	bool load(const char* fname) {
		std::ifstream* _f = new std::ifstream(fname);
		if (_f->is_open()) {
			std::string* _s = new std::string("");
			*_f >> *_s;
			*_f >> *_s;
...
class XsShapeX {
private:
	uint_fast8_t* _vert_type = nullptr; // 0 - vertices, 1 - vertex, 2 - solid
	void* _vert = nullptr;
	GLenum* _gl = nullptr;
	XsEnum* _xs = nullptr;
	XsSolidType* _sl = nullptr;
public:
	vex3f pos = 0.f;
	vex3f rot = 0.f;
	vex4f color = 1.f;
	vex3f scale = 1.f;
	XsEnum& XsMode() { return *_xs; };
	GLenum& GLmode() { return *_gl; };
	XsSolidType& SolidType() { return *_sl; };
	const char* VertType() { return *_vert_type == 0 ? "vertices" : *_vert_type == 1 ? "vertex" : "solid"; };
	XsTextureSTB tex;
	void set(XsVertices v) {
		_vert_type = new uint_fast8_t(0);
		_gl = new GLenum(GL_TRIANGLES);
		_xs = new XsEnum(XS_VERTEX);
		_sl = nullptr;
		_vert = &v;
	};
	void set(XsVertices v, GLenum gl) {
		_vert_type = new uint_fast8_t(0);
		_gl = new GLenum(gl);
		_xs = new XsEnum(XS_VERTEX);
		_sl = nullptr;
	...
	void set(XsVertices v, XsEnum xs) {
		_vert_type = new uint_fast8_t(0);
		_gl = new GLenum(GL_TRIANGLES);
		_xs = new XsEnum(xs);
		_sl = nullptr;
	...
	void set(XsVertices v, GLenum gl, XsEnum xs) {
		_vert_type = new uint_fast8_t(0);
		_gl = new GLenum(gl);
		_xs = new XsEnum(xs);
		_sl = nullptr;
	...
	void set(XsVertices v, XsEnum xs, GLenum gl) {
		_vert_type = new uint_fast8_t(0);
		_gl = new GLenum(gl);
		_xs = new XsEnum(xs);
		_sl = nullptr;
	...
	void set(XsVertexDepo v) {
		_vert_type = new uint_fast8_t(1);
		_gl = new GLenum(GL_TRIANGLES);
		_xs = new XsEnum(XS_VERTEX);
		_sl = nullptr;
		_vert = &v;
	};
	void set(XsVertexDepo v, GLenum gl) {
		_vert_type = new uint_fast8_t(1);
		_gl = new GLenum(gl);
		_xs = new XsEnum(XS_VERTEX);
		_sl = nullptr;
	...
	void set(XsVertexDepo v, XsEnum xs) {
		_vert_type = new uint_fast8_t(1);
		_gl = new GLenum(GL_TRIANGLES);
		_xs = new XsEnum(xs);
		_sl = nullptr;
	...
	void set(XsVertexDepo v, GLenum gl, XsEnum xs) {
		_vert_type = new uint_fast8_t(1);
		_gl = new GLenum(gl);
		_xs = new XsEnum(xs);
		_sl = nullptr;
	...
	void set(XsVertexDepo v, XsEnum xs, GLenum gl) {
		_vert_type = new uint_fast8_t(1);
		_gl = new GLenum(gl);
		_xs = new XsEnum(xs);
		_sl = nullptr;
	...
	void set(XsSolidType v, XsEnum xs = XS_VERTEX) {
		_vert_type = new uint_fast8_t(2);
		_gl = nullptr;
		_xs = new XsEnum(xs);
		_sl = new XsSolidType(v);
		_vert = nullptr;
	};
	void draw() {
		tex.bind();
		...
		glColor4f(color);
		glScalef(scale);
		*_vert_type == 0 ? XsPrintVertices(*static_cast<XsVertices*>(_vert), XsEnum(*_xs), GLenum(*_gl))
			: *_vert_type == 1 ? XsPrintVertex(*static_cast<XsVertexDepo*>(_vert), *_xs, *_gl)
			: XsSolid(*_sl, *_xs);
	};
};
...
void XsDrawColl(XsCollission v, float s, vex3f color) {
	...
	glScalef(v.scale.x, v.scale.y, v.scale.z);
