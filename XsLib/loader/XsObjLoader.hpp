bool XsOBJLoader(const char* __file_name, XsVertexDepo& __vertices) {
	...
	if (!__in_file.is_open())
	{
		...
		__vertices.resize(__vertex_position_indicies.size(), XsVertex(vex3f(0), vex2f(0), vex3f(0)));
...
XsVertices XsOBJLoader(const char* __file_name, XsEnum mode) {
	return XsVerticesLoad(XsOBJLoader(__file_name), mode);
};
...
bool XsOBJShape::loadFromFile(const char* __file_name) {
	...
	if (!__in_file.is_open())
	{
		...
		vertices.resize(__vertex_position_indicies.size(), XsVertex(vex3f(0), vex2f(0), vex3f(0)));
