class XsModel {
	...
	int __size = 0;
public:
	GLenum glMode() { return __mode; };
	GLenum xsMode() { return XsEnum(__Xmode); };
	XsModel(const char* file_name) { loadFromFile(file_name); };
...
XsVertices XsModelLoader(const char* file_name) { return XsModel::XsModel(file_name).vertices; };
void XsModelLoader(XsVertices& _v, const char* file_name) { _v = XsModel::XsModel(file_name).vertices; };